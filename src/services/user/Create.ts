...
export class CreateUserService extends BaseService implements CreateUserServiceImp {
    ...
    constructor(createUserRepository: CreateUserRepositoryImp, findUserByEmailRepository: FindUserByEmailRepositoryImp) {
        super()
    ...
    public async execute(user: UserImp): Promise<UserImp> {
        const userAlreadyRegistered = await this._findUserByEmailRepository.execute(user.email)
